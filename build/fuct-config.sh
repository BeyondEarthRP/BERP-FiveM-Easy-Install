...
harvest() {
	...
	# SERVICE_PASSWORD
	if [ "$__CONFIGURE__" ] || [ ! "$SERVICE_PASSWORD" ] ;
	then
	...
	# DB_ROOT_PASSWORD
	if [ "$__CONFIGURE__" ] || [ ! "$DB_ROOT_PASSWORD" ] ;
	then
	...
	# MYSQL_PASSWORD
	if [ "$__CONFIGURE__" ] || [ ! "$MYSQL_PASSWORD" ] ;
	then
	...
	# BLOWFISH_SECRET
	if [ "$__CONFIGURE__" ] || [ ! "$BLOWFISH_SECRET" ] ;
	then
	...
	# STEAM_WEBAPIKEY
	if [ "$__CONFIGURE__" ] || [ ! "$STEAM_WEBAPIKEY" ] ;
	then
	...
	# SV_LICENSEKEY
	if [ "$__CONFIGURE__" ] || [ ! "$SV_LICENSEKEY" ] ;
	then
	...
	# RCON
	if [ "$__CONFIGURE__" ] || [ ! "$RCON_ENABLE" ] ;
	then
		PROMPT="Enable RCON (probably not needed)?"
		pluck_fig "RCON_ENABLE" 10 false
		_all_new_+=("RCON_ENABLE")
	fi
	[[ "$RCON_ENABLE" == "true" ]] && RCON="$RCON_ENABLE"
	if [ "$RCON_ENABLE" == "true" ] ;
        then
		# RCON_PASSWORD_GEN
		if [ "$__CONFIGURE__" ] || [ ! "$RCON_PASSWORD_GEN" ] ;
		then
		...
		then
			...
			if [ "$__CONFIGURE__" ] || [ ! "$RCON_PASSWORD_LENGTH" ] ;
			then
			...
			# RCON_ASK_TO_CONFIRM
			if [ "$__CONFIGURE__" ] || [ ! "$RCON_ASK_TO_CONFIRM" ] ;
			then
			...
			# RCON_PASSWORD
			if [ "$__CONFIGURE__" ] || [ ! "$RCON_PASSWORD" ] ;
			then
	...
	if [ "$__CONFIGURE__" ] || [ ! "$TXADMIN_BACKUP_FOLDER" ] ;
	then
	...
	# DB_BACKUP_FOLDER
	if [ "$__CONFIGURE__" ] || [ ! "$DB_BACKUP_FOLDER" ] ;
	then
	...
	# ARTIFACT_BUILD
	if [ "$__CONFIGURE__" ] || [ ! "$ARTIFACT_BUILD" ] ;
	then
	...
	# SOFTWARE_ROOT
	if [ "$__CONFIGURE__" ] || [ ! "$SOFTWARE_ROOT" ] ;
	then
	...
	if [ "$TXADMIN_BACKUP" ] && [ "$TXADMIN_BACKUP" != "$PRIVATE/$TXADMIN_BACKUP_FOLDER" ] ;
	then
		TXADMIN_BACKUP="$PRIVATE/$TXADMIN_BACKUP_FOLDER"
		_all_new_+=("TXADMIN_BACKUP")
	fi

	if [ "$DB_BACKUPS" ] && [ "$DB_BACKUPS" != "$PRIVATE/$DB_BACKUP_FOLDER" ] ;
	then
...
define_configures() {
        ...
        [[ ! "$SOFTWARE_ROOT" ]] &&  echo "Configures error @ SOFTWARE_ROOT"  && exit 1
          TFIVEM="${TFIVEM:=$SOFTWARE_ROOT/fivem}"
          TCCORE="${TCCORE:=$TFIVEM/citizenfx.core.server}"

        [[ ! "$SERVICE_ACCOUNT" ]] && echo "Configures error @ SERVICE_ACCOUNT" && exit 1
...
cook_figs() {
	...
	then
		...
		jq ". += {\"sys\":{}}"     	                                          	       			| \
			...
			jq ".sys += {\"rcon\":{}}"                                                              	| \
				jq ".sys.rcon += {\"enable\":\"${RCON_ENABLE))}\"}"                                   	| \
